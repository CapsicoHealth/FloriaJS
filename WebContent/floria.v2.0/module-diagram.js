...
// Ensure JointJS CSS is loaded (required for correct element rendering).
if (!document.getElementById('jointjs-css')) {
  const link = document.createElement('link');
  link.id   = 'jointjs-css';
  link.rel  = 'stylesheet';
  link.href = 'https://cdn.jsdelivr.net/npm/@joint/core@4.0.4/dist/joint.css';
...
FloriaDiagram.prototype._createCanvasBoundary = function() {
  ...
  this._paper.drawBackground({ color: '#f0f2f5' });
  ...
  rect.setAttribute('stroke', '#9aa3b0');
...
FloriaDiagram.prototype.add = function(item) {
  ...
  const element = new joint.shapes.standard.Rectangle({
    ...
    attrs: {
      body: { fill: '#ffffff', stroke: '#333', strokeWidth: 1, rx: 5, ry: 5 },
...
FloriaDiagram.prototype.fullscreen = function(toggle) {
  ...
  if (shouldBe && !this._isFullscreen) {
    this._originalStyles = { position: this._container.style.position, top: this._container.style.top,
      left: this._container.style.left, width: this._container.style.width, height: this._container.style.height, zIndex: this._container.style.zIndex };
    Object.assign(this._container.style, { position: 'fixed', top: '0', left: '0', width: '100vw', height: '100vh', zIndex: '9999' });
