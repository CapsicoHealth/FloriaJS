...
DataFetcherRegistry.registerSimpleFetcher = function(fetcherName, paramDefs, cacheBuster, url, errMsg)
{
  var f = function() {
    this.init = function(params)
     {
       this._params = FloriaDOM.checkParams("DataFetcher '"+fetcherName+"'", paramDefs, params);
       if (this._params == null)
        {
          console.log("#####################################################################################################")
          console.log("DataFetcher '"+fetcherName+"' was not initialized properly.");
          console.log("#####################################################################################################")
        }
     }
    if (cacheBuster == true)
      this.softCacheBuster = new Date().getTime();
    this._webService= url;
    this.getDescription = function() { return "Web service "+this._webService; };
    this.getData = function(callbackFunc)
     {
       var urlParams = FloriaDOM.makeUrlParams(this._params);
       FloriaAjax.ajaxUrl(this._webService+urlParams+"&ts="+this.softCacheBuster, "GET", 
                          null, 
                          function(data) { callbackFunc(data==null?null:data, errMsg); },
                          function(    ) { callbackFunc(null, errMsg); }
                         );
     };
  }
  DataFetcherRegistry.register(fetcherName, { }, f);
  return f;
}
   ...
   if (rowPainter == null)
    {
      ...
      wrapper.innerHTML= '<TABLE><TR><TD><img src="/static/img/error.gif" height="20px"> RowPainter \''+rowPainterName+'\' not found or initialized properly.</TD></TR></TABLE>';
      return wrapper;
    }
   rowPainter = rowPainter._obj;
   rowPainter._elementId = elementId;
       ...
       for (var i = 0; i < links.length; ++i)
        {
          ...
          if (l == null)
           continue;
          let title = l.alt || l.title;
          if (l.markup==null)
            Str+='<A href="javascript:'+l.action+'" '+(title==null?'':'alt="'+title+'" title="'+title+'"')+'><img src="'+l.img+'"></A>';
           else
...
function renderTile(layoutId, compDef, rowLength)
  {
    ...
    Str+='<DIV id="'+comp._obj._tileId+'" class="tileContents '+(compDef.height=='fit-content'?'':'overflowable')+'" style="'
                      +(expand == 0 ? (comp._obj._minimized=true, 'display:none;'):'')
                      +(compDef.height != null?'height:'+compDef.height+' !important;max-height:'+compDef.height+' !important;':compDef.maxHeight != null?'max-height:'+compDef.maxHeight+' !important;':'')
                      +'">'
...
//         }
       ...
       DataFetcherRegistry.runAll(function() { 
          ...
          if (FloriaDOM.isFunction(afterRenderCallbackFunc) == true)
            afterRenderCallbackFunc();
          else if (afterRenderCallbackFunc == true && onChangeCallbackFunc != null)
  ...
  if (picker._internal.values != null && picker._internal.values.length > 0)
    {
     ...
     {
       temp = '<IMG src="/static/img/warning.gif" style="height:1em; padding-right:.25em;" align="absmiddle">No selection';
       Str1 = temp;
       Str2 = temp;
     }       
