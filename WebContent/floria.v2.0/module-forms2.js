...
function makeUpdatePageFunc(that, toPageId, reset) 
  { 
    return function() { if (reset == true) that._pageId = null; that.updatePage(toPageId); };
  }
   ...
   if (d.type!="hidden")
    {
      ...
      if (d.inline == true || verticalLayout == true)
        ...
        Str+='<TD class="fieldName'+(d.mandatory == true ? " mandatory":"")+'" '+(d.description==null?"":'title="'+d.description+'"')+'>'+(d.label==null?'':d.label)+'</TD><TD colspan="9" id="'+elementId+'_'+pageId+'_PH'+xid+'">';
    }
   if (d.type=="text")
    Str+='<INPUT name="'+fieldName+'" type="text" value="'+FloriaText.TextUtil.printFuncParam(v)+'" style="width:'+(d.width==null?'90%':d.width)+'"'+(d.placeHolder==null?'':' placeholder="'+d.placeHolder+'"')+(d.readOnly==true?' readonly':'')+'>';
   else if (d.type=="hidden")
    Str+='<INPUT name="'+fieldName+'" type="hidden" value="'+FloriaText.TextUtil.printFuncParam(v)+'">';
   else if (d.type=="file")
    Str+='<INPUT name="'+fieldName+'" type="file" style="width:90%" '+(d.multi||d.multiple?'multiple="true"':'')+' '+(d.accept!=null?'accept="'+d.accept+'"':'')+'>';
   else if (d.type=="textarea")
    Str+='<TEXTAREA name="'+fieldName+'" rows="'+(d.rows==null?10:d.rows)+'" style="width:'+(d.width==null?'90%':d.width)+'"'+(d.placeHolder==null?'':' placeholder="'+d.placeHolder+'"')+(d.readOnly==true?' readonly':'')+'>'+(FloriaText.isNoE(v) == true ? "" : FloriaText.print(v))+'</TEXTAREA>';
...
//     Str+='<INPUT type="hidden" id="'+elementId+'_a_'+xid+'" name="'+fieldName+'" value="'+FloriaText.TextUtil.printFuncParam(v)+'"><DIV id="'+elementId+'_b_'+xid+'"><DIV>';
    ...
    Str+=FloriaControls.Dropdown.gen(null, formElementIds, d.values, d.firstEmpty==true?true:false, null, v, d.multi||d.multiple, d.size);
...
function fillField(d, elementId, pageId, rowId, subRowId, descriptions, data, pickers, groupCount, customRenderers)
 {
   ...
   if (d.type=="text")
    ...
    {
      let v = data == null ? null : data[d.name];
      if (v == null)
       v = d.defaultValue; 
      else if (v.length > 0)
       v = v[0];
...
//      console.log(v, ", ", d.defaultValue);
      ...
      if (v != null && Array.isArray(v) == false)
       v = [ v ] ;
      pickers.push({fieldName: d.name, pickerName:d.type, elementId:elementId+"_"+pageId+"_PH"+xid, name:fieldName, multi:d.multi||d.multiple
...
export var FloriaForms = function(elementId, data, formDefs, edgeColumnCount, processCallbackFunc, popupTitle, wizardMode, liveOnChange, persist, validationErrMsg, cancelButton, submitButton, testingMode=false)
 { 
...
//   console.log("=>=>=> Forms2.constructor (this._data): ", this._data);
   ...
   for (var i = 0; i < this._formDefs.length; ++i)
    {
      ...
      for (var j = 0; j < d.length; ++j)
       {
         var f = d[j];
         if (f.type=="checkbox" || f.type=="radio" || f.type=="dropdown" || f.type=="combo")
          {
            if (f.values == null)
   ...
   this.getFieldValueLabels = function(values, fieldValues)
    {
      ...
      for (var v = 0; v < values.length; ++v)
       for (var f = 0; f < fieldValues.length; ++f)
        if (fieldValues[f] != null && fieldValues[f].length >= 2 && values[v] == fieldValues[f][0])
   ...
   this.triggerOnChange = function()
    {
      let savedPageId = this._pageId;
      this._pageId = null;
      this.paint(savedPageId);
      this.updatePage();
    }
      ...
      for (var i = 0; i < p.length; ++i)
       {
         ...
         if (d.name==null)
          {
            if (d.label==null && d.description==null)
              Str+='<TR><TD colspan="10">&nbsp;</TD></TR>';
            else
              {
                Str+='<TR><TD colspan="10">'+(d.label==null?'':'<DIV class="formLabelName">'+d.label+'</div>')
                    +(d.description==null?'':'<div class="formLabelDescr">'+d.description+'</div>')
                    +'</TD></TR>'
                    ;
                heading = true;
              }
...
//                                  console.log("=>=>=>  values: ", d2.name, values);
               ...
               else if (d.name == null) // Nothing to do for labels
                {
                 ...
                 {
                   if (f[d.name] != null)
                    this._data[d.name] = f[d.name].value.split("``");
                   else
            ...
            if (MissingParams.length > 0 && skipValidation == false && this._testingMode == false)
             {
               for (var i = 0; i < p.length; ++i)
                {
                  var d = p[i];
                  ...
                  if (MissingParams.indexOfSE(d.name) == -1)
                   FloriaDOM.removeCSS(id, "formError");
                  else
                   FloriaDOM.addCSS(id, "formError");
                }
                         ...
                         if (f.type=="date")
                          ...
                          v = FloriaText.TextUtil.replaceNewLinesWithBreaks(v, false);
                         else if (Array.isArray(v) == true && v.length > 0)
                           {
                             ...
                             var pickerDef = {fieldName: field.name, pickerName: field.type, elementId: elementId, multi: field.multi||field.multiple, params: field.params, values: v, descriptions: this._descriptions[field.name]}
                 ...
                 if (FloriaText.isNoE(val) == false || showAllElements == true)
                  {
                    ...
                    if (header2!=null && label2 != label1) Str+=header2;
                     ...
                     if (f.label != null)
                      {
                        if (f.label.length > maxLabelLengthBeforeWrapping || this.verticalLayout == true)
                          {
      ...
      if (editable == true)
       {
         var that = this;
         for (var i = 0; i < this._formDefs.length; ++i)
          FloriaDOM.addEvent(this._elementId+'_P'+i, "click", makeUpdatePageFunc(that, i, true), null, true);
       }
   ...
   for(var i = 0; i < fieldNames.length; ++i)
    {
       ...
       if (FloriaText.isNoE(value) == true)
        {
          FloriaDOM.addCSS(e, "formError");
          OK = false;
        }
       else
        {
          def[elementName] = value;
          FloriaDOM.removeCSS(e, "formError");
        }
    }
   return OK;
 }
          ...
          for (var i = 0; i < 100; ++i)
           {
             ...
             FloriaDOM.removeCSS(valueElement, "formError");
             FloriaDOM.removeCSS(labelElement, "formError");
             if (FloriaText.isNoE(value) == false && FloriaText.isNoE(label) == false)
             ...
             else if (FloriaText.isNoE(value) == false || FloriaText.isNoE(label) == false)
              {
                OK = false;
                if (FloriaText.isNoE(value) == true)
                 FloriaDOM.addCSS(valueElement, "formError");
                if (FloriaText.isNoE(label) == true)
                 FloriaDOM.addCSS(labelElement, "formError");
              }
           }
          if (values.length > 0)
           def.values = values;
           ...
           {
             OK = false;
             FloriaDOM.addCSS(e["value0"].parentNode, "formError");
             FloriaDOM.addCSS(e["label0"].parentNode, "formError");
           }
        }
    
      if (OK == false && this._testingMode == false)
