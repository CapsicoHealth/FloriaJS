...
export var FloriaDOM = {
   ...
   getAncestorNode: function(e, nodeName, dataAttribute)
      {
        nodeName = nodeName.toUpperCase();
        e = FloriaDOM.getElement(e);
        while (e != null && e.nodeName.toUpperCase() != nodeName)
          e=e.parentNode;
        if (e != null && dataAttribute != null && e.dataset[dataAttribute] == null)
         return this.getAncestorNode(e.parentNode, nodeName, dataAttribute);
        return e;
      },
   getAncestorNodeByClass: function(e, className)
    ...
    setInnerHTML: function(e, str, throwMsg, maxRetriesOf200ms=0, ifEmptyOnly=false, scrollTop=false)
      {
        let counter = 0;
        let func = function() {
           let elem = FloriaDOM.getElement(e);
           if (elem == null)
            {
              if (++counter > maxRetriesOf200ms)
               { 
                 console.error("Cannot find element '"+e+"' to set innerHTML.")
                 return throwMsg != null ? FloriaDOM.alertThrow(throwMsg) : null;
               }
              return setTimeout(func, 200);
            }
           if (ifEmptyOnly == false || FloriaText.isNoE(elem.innerHTML) == true)
            {
     ...
     execScriptBlocks: function(e, throwMsg)
      {
        e = FloriaDOM.getElement(e, throwMsg);
     ...
     setValue: function(elementId, value, throwMsg)
      {
        let e = FloriaDOM.getElement(elementId, throwMsg);
    ...
    insertBefore: function(target, newElement)
      {
        var e = FloriaDOM.getElement(target);
        if (e != null)
         e.parentNode.insertBefore(newElement, e);        
      },
    ...
    removeCSSFromAllSiblings: function(e, className, setCurrent, Str)
      {
        var e = FloriaDOM.getElement(e, Str);
        if (e == null)
         return;
        let siblings = e.parentNode.getElementsByClassName(className);
        if (siblings != null)
         for (let i = 0; i < siblings.length; ++i)
          {
            let s = siblings[i];
            if (s != e && s.classList != null)
    ...
    isElementVisible: function(e, str)
     {
       var e = FloriaDOM.getElement(e, str);
       if (e == null)
        return false;
       ...
       if (topElement != null && topElement.style.pointerEvents!="none" && e.contains(topElement)==false) // is the element covered?
        return false;

       // is the element, or any of its parents, hidden?
       while (e != null && e.nodeType == 1 && e.tagName != "BODY")
        {  
...
// console.log("Starting waiter "+this.waiterName+"."+this.elementId);
            ...
            this.execute = function()
             {
               ...
               if (e == null)
                ...
                return this.callbackFunc(true, e);
             };
            this.execute();
        };
      },
     /** Creates a new object with the property of Obj1. then override and adds properties from Obj2, and then Obj3.
         In all cases, if a property exists in all three, only the latest value will survive.
      */
     ...
     hasFiles:function(Obj)
      {
        var props = Object.keys(Obj);
        for (var i = 0; i < props.length; ++i)
         {
           let d = Obj[props[i]];
           if (d instanceof FileList || d instanceof File)
            return true;
         }
        return false;
      },
     ...
     addEvent: function(elementId, eventName, handlerFunc, delayedMillis, clearPrevious)
      {
        ...
        if (clearPrevious == true)
         {
           FloriaDOM.removeEvents(e, eventName);
           EVENT_REGISTRY[elementId+"_"+eventName] = func;
         }
        e.addEventListener(eventName, func);
        return func;
      },
     fireEvent: function(elementId, eventName, bubbles=true, cancelable=true)
      {
        var e = this.getElement(elementId);
        if (e == null)
         return console.error("Cannot fire event on element '"+elementId+"' because it cannot be found.");
        const evt = new Event(eventName, { bubbles: bubbles, cancelable: cancelable });
        e.dispatchEvent(evt);
      },
     removeEvents: function(e, eventName)
      {
        var oldFunc = EVENT_REGISTRY[e.id+"_"+eventName];
        if (oldFunc != null)
         {
           e.removeEventListener(eventName, oldFunc);
           delete EVENT_REGISTRY[e.id+"_"+eventName];
         }
      },
    ...
    createElementOverlay: function(e, className=null, preExistingOverlay=null, badgeNumber=null)
     {
       let rect = e.getBoundingClientRect();
       let dims = { top: rect.top-2, left: rect.left-2, width: rect.right-rect.left+2, height: rect.bottom-rect.top+2 };

       let overlay = preExistingOverlay!=null ? preExistingOverlay : document.createElement("DIV");
       ...
       if (preExistingOverlay == null)
        {
          let overlays = document.getElementById("FLORIA_OVERLAYS");
          if (overlays == null)
            {
              overlays = document.createElement("DIV");
              overlays.id = "FLORIA_OVERLAYS";
              overlays.classList.add("floriaOverlays");
    ...
    areOverlaysVisible: function()
     {
       let overlays = document.getElementById("FLORIA_OVERLAYS");
       return overlays != null && overlays.style.display != "none";
     },
    clearAllOverlays: function()
     ...
     getElementDimensions: function(elementId)
      {
        return   "top:"+elementId.style.top
              +", left:"+elementId.style.left
              +", bottom:"+elementId.style.bottom
              +", right:"+elementId.style.right
              +", width:"+elementId.style.width
              +", height:"+elementId.style.height
              +", opacity:"+elementId.style.opacity
              +", transform:"+elementId.style.transform
              ;
      },
...
// "+document.documentElement.clientWidth+";");
     ...
     localStorageRemove: function(id)
      {
    ...
    ,Timer: function(divId, frequenceMillis, statusDivId, servletPath)
      {
        this._done = false;
           ...
           if (that._statusDiv != null)
            {
              ...
              if (data?.message != null)
               that._statusDiv.innerHTML = data.message;
            }
           setTimeout(func, frequenceMillis);
         }
        setTimeout(func, frequenceMillis);
      }
    ,getRootDomain : function(url) // returns the root domain assuming that if there are two dots, the root domain are the last 2 parts, i.e., xyz.com for a URL such as www.xyz.com
